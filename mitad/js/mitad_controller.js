var app = angular.module('mitadApp', []);
app.controller('mitadCtrl', function($scope, $window) 
{
     ...
     */
    let getNumbers2Do = ((numElements, operationType, numPositions, level, decimals) => 
    {
        let returnObj = [];
           ...
           level && level > 0)
        {
            ...
            for(let i=0; i < numElements; i++)
            {
                ...
                let numberFormat = new Intl.NumberFormat('es-ES', { minimumFractionDigits: 2 });
                returnObj.push({ "value": numberFormat.format(n), "result": r })
            }
        }
     ...
     */ 
     $scope.validaResultados = (() => {
        ...
        {
            let userResult = $scope.result.line[i][1].replace(",", ".");
            let showError = true;
    ...
    let numElements = 5
    let operationType = "M" // "D" - Doble | "M" - Mitad
    let numPositions = 2;
    let decimals = true;
    ...
    $scope.numberList = getNumbers2Do(numElements, operationType, numPositions, level, decimals);
